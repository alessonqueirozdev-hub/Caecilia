 ...
 *          dependency into @c caecilia_core. A concrete socket backend lands as
...
public:
     ...
     *        @c ControlCommand.
     * @param address The OSC address pattern (e.g. `/caecilia/stops/engage`).
    ...
    /// @return the OSC address for @p opcode (e.g. `/caecilia/stops/engage`).
    [[nodiscard]] static std::string addressFor(ControlOpcode opcode);

private:
    /// Encode @p event as the outbound `/caecilia/state/...` bundle (TODO: emit).
