...
The scheduler and voice-lifecycle layer of the pure `caecilia_core` library. It
...
This module lives inside `caecilia_core`. It depends **only** on the core
...
(`dsp` → `caecilia::dsp`, `wind` → `caecilia::wind`, and so on). The engine
module is the **one intentional exception**: its symbols live in
`caecilia::core::engine`, nested under `core`. The engine *is* the real-time
