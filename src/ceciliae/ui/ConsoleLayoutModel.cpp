...
void ConsoleLayoutModel::layoutFrom(const model::Organ& organ, LayoutRect canvas)
{
    ...
    for (const auto& division : divisions)
    {
        ...
        header.semantic.oscAddress = "/ceciliae/" + slugify(division.name());
        ...
        for (const core::StopId sid : division.stops())
        {
            ...
            e.semantic.oscAddress = "/ceciliae/" + slugify(division.name())
                                    + "/" + slugify(stop->name());
    ...
    for (const auto& division : divisions)
    {
        ...
        kb.semantic.oscAddress = "/ceciliae/" + slugify(division.name()) + "/keys";
    ...
    for (const auto& coupler : couplers)
    {
        ...
        e.semantic.oscAddress = "/ceciliae/coupler/" + slugify(coupler.name());
        elements_.push_back(e);
    ...
    for (std::uint16_t p = 1; p <= 8; ++p)
    {
        ...
        e.semantic.oscAddress = "/ceciliae/general/" + std::to_string(p);
        elements_.push_back(e);
    ...
    expr.semantic.oscAddress = "/ceciliae/expression/1";
    expr.learnable = true;
