...
# `ceciliae::ui` — the console UI pillar

The `ui` module is Ceciliae's **first-class user-interface pillar** (benchmark:
...
It contributes its sources to the `Ceciliae` JUCE target, not to the pure
`ceciliae_core` static library, and it depends on `core`, `engine`, `model` and
...
| Type | Role |
|------|------|
| `CeciliaeLookAndFeel` | Custom look-and-feel for **standard JUCE dialogs only** (the console is painted by skins, not the LnF) |
...
`RegistrationPanel` is the visual face of Ceciliae's registration moat. Rather
...
(`v0.7`). The module compiles into the `Ceciliae` plugin target only; the
