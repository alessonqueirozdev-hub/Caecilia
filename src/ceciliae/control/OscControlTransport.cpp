...
#include "ceciliae/control/OscControlTransport.h"
...
constexpr std::string_view kRoot = "/ceciliae/";
...
bool OscControlTransport::start()
{
    ...
    // library is linked into ceciliae_core; the concrete backend is an isolated
...
std::string OscControlTransport::encodeFeedback(const StateEvent& event) const
{
    ...
    // documents the intended /ceciliae/state/... mapping and keeps the seam live.
