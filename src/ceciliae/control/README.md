...
hardware, accessibility tech — can drive Ceciliae's registration brain and
playback the same way the UI does.

This module contributes to the pure `ceciliae_core` static library
...
`StateDelta` emission) lives in `ceciliae::registration` behind the
...
ceciliae::control::ControlServer server{sink};

ceciliae::control::LoopbackTransport loop;
server.addTransport(loop);   // connects it + subscribes it for feedback
server.start();

using ceciliae::control::ControlCommand;
...
These stubs pull **no** third-party socket or JSON dependency into `ceciliae_core`
