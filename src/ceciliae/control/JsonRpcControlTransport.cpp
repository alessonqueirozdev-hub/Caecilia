...
#include "ceciliae/control/JsonRpcControlTransport.h"

namespace ceciliae::control
{
namespace
{
constexpr std::string_view kMethodPrefix = "ceciliae.";
} // namespace
...
bool JsonRpcControlTransport::start()
{
    ...
    // JSON dependency is linked into ceciliae_core; the concrete backend is an
...
std::string JsonRpcControlTransport::encodeNotification(const StateEvent& event) const
{
    ...
    (void) event;
    return R"({"jsonrpc":"2.0","method":"ceciliae.event"})";
}
