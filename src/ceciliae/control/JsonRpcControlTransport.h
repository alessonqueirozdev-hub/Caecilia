 ...
 *        codec backend).
 *
 * Exposes the command surface as JSON-RPC 2.0 methods named `ceciliae.<token>`,
 ...
 *          @c ceciliae_core. A concrete codec + socket/websocket backend lands as
...
public:
    ...
    /// @return the JSON-RPC method name for @p opcode (e.g. `ceciliae.stops.engage`).
    [[nodiscard]] static std::string methodFor(ControlOpcode opcode);

    /// @return the opcode for a `ceciliae.<token>` @p method, or @c None if unknown.
...
private:
    ...
    /// Encode an outbound `ceciliae.event.*` notification (TODO: real JSON serialise).
