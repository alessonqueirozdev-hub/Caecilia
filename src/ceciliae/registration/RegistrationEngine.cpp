                                                    ...
                                                    const RegistrationCommand& command) const
{
    ...
    switch (command.kind)
    {
        ...
        case Kind::ApplyIntent:
            if (spec_ != nullptr)
            {
                const PortResult pr = ::ceciliae::registration::port(command.intent, *spec_);
...
PortResult RegistrationEngine::port(const RegistrationIntent& intent) const
{
    if (spec_ == nullptr)
        return PortResult{};
    return ::ceciliae::registration::port(intent, *spec_);
}
