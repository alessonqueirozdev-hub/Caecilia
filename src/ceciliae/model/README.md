...
# `ceciliae::model` — the instrument definition

The `model` module is Ceciliae's **instrument description with first-class
...
It is part of the pure `ceciliae_core` static library: it depends only on
...
loader, so `ceciliae_core` never touches the filesystem.
...
| `SampleSetDescriptor` | Ceciliae's **own** proprietary sample-format descriptor (never a GPL'd GO/HW set) | — |
...
## Organ file schema (Ceciliae's own format)
...
> Ceciliae never ingests a GPL'd GrandOrgue / Hauptwerk organ definition or
